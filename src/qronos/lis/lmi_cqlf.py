...
def cqlf(A, Delta_list, rho, beta, R_inv='auto'):
    ...
    rho: float: desired upper bound for P_norm(A). Must be larger than the spectral radius of A.
        Increasing rho decreases the above bound, but typically improves robustness, i.e., tends to lower P_norm(Delta) for most Delta
    ...
    R_inv: n-by-n np.array or iv.matrix or None or 'auto':
        ...
        Output is roughly the same (but with lower numerical error if R is chosen wisely).
    ...
    # CVXOPT with kktsolver='robust' turned out to be the most robust open-source option available
